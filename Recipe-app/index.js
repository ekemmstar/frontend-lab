...
document.getElementById('searchBtn').addEventListener('click',()=>{
    const userIn = document.getElementById('searchInput').ariaValueMax.trim();
    if (userIn !== ''){
        searchFn(userIn)
    }else{
...
function showRecipesFn(recipes){
    const recipeList = document.getElementById('recipeContainer');
    recipeList.innerHTML = '';
    recipes.slice(0, 20).forEach(recipe => {
        const recipeItem = document.createElement('div');
        recipeItem.classList.add('animate__animated', 'animate__fadeIn', 'recipe-card');
